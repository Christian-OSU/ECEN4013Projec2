...
String time, fix, sat, lat, longi, elevation;
...
void setup() {
  ...
  lastPushTime = millis();
  startBtn = new Button(20, height - 50, 80, 30, "Start");
  stopBtn  = new Button(120, height - 50, 80, 30, "Stop");

}

void draw() {
  background(30);
  if (running && (myPort == null || !myPort.active())) {
  println("⚠ Serial connection lost. Exiting...");
  exit();
}
...
if (startBtn.isClicked()) {
  ...
  if (myPort == null) {
    try {
      ...
      myPort.bufferUntil('\n');
      println("Reconnected serial port.");
    } catch (Exception e) {
      println("Error reopening serial: " + e.getMessage());
    }
  }
}
...
if (stopBtn.isClicked()) {
  ...
  try {
    if (myPort != null) {
      myPort.clear();
      myPort.stop();   // closes the port
      myPort = null;   // prevent reuse after stop
      println("Serial port closed cleanly.");
    }
  } catch (Exception e) {
    println("⚠ Error closing serial port: " + e.getMessage());
  }
}
  ...
  if (!running) {
    ...
    textSize(20);
    text("Press 'Start' to begin reading data", width/2, height/2);
    return; // don't draw charts or update data
  }
  ...
  text("Elevation: " + elevation, 10, y); y+=24;
...
void serialEvent(Serial myPort) {
  if (!running) return;  // ignore if stopped
  ...
  try {
    ...
    if (inString.startsWith("Orientation")) {
      // Example:
      // Orientation: 77.38, 158.50, 0.00; Angular Velocity: -0.12, -0.06, 0.00;
      // Linear Acceleration: 0.00, -0.01, 0.00; Magnetic Field: 22.06, -47.38, -52.75

      String[] groups = split(inString, ";");
      for (String g : groups) {
        g = trim(g);
        if (g.startsWith("Orientation")) {
          float[] vals = float(split(trim(split(g, ":")[1]), ","));
          if (vals.length == 3) {
            yaw = vals[2];
            pitch = vals[0];
            roll = vals[1];
          }
        } else if (g.startsWith("Angular Velocity")) {
          float[] vals = float(split(trim(split(g, ":")[1]), ","));
          if (vals.length == 3) {
            gyroX = vals[0];
            gyroY = vals[1];
        ...
        } else if (g.startsWith("Linear Acceleration")) {
          float[] vals = float(split(trim(split(g, ":")[1]), ","));
          if (vals.length == 3) {
            accelX = vals[0];
            accelY = vals[1];
        ...
        } else if (g.startsWith("Magnetic Field")) {
          float[] vals = float(split(trim(split(g, ":")[1]), ","));
          if (vals.length == 3) {
            magX = vals[0];
            magY = vals[1];
    ...
    // ---------- GPS DATA ----------
      ...
      // Example:
      // Time: 0:21:3; Fix: 0; Satellites: 0; Latitude: ---; Longitude: ---; Elevation: ---
      ...
      for (String g : groups) {
        ...
        if (g.startsWith("Time")) {
          String[] parts = split(trim(split(g, ":")[1]), ":");
          if (parts.length >= 3) {
            time = parts[0] + ":" + parts[1] + ":" + parts[2];
          } else time = trim(split(g, ":")[1]);
        } 
        else if (g.startsWith("Fix")) {
          fix = (trim(split(g, ":")[1]));
        } 
        else if (g.startsWith("Satellites")) {
          sat = (trim(split(g, ":")[1]));
        } 
        else if (g.startsWith("Latitude")) {
          lat = trim(split(g, ":")[1]);
        } 
        else if (g.startsWith("Longitude")) {
          longi = trim(split(g, ":")[1]);
        } 
        else if (g.startsWith("Elevation")) {
          elevation = trim(split(g, ":")[1]);
        }
      }
    }
  } 
